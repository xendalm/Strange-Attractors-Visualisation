...
class Visualisation {
  ...
  public:
	void init(const vector<point_3d>& points, int _step = -1, float _rotation = 0.25,
			  bool set_scale_for_each_axis = false) {
		if (points.size() < 1) {
			cout << "Empty vector";
			exit(EXIT_FAILURE);
		}
		...
		step = _step;
		if (step == -1) cur_index = iterations;
		rotation = _rotation;
		...
		if (set_scale_for_each_axis) {
			...
			float k = float(1) / max(max((abs(max_x - min_x) / 2), abs(max_y - min_y) / 2),
									 abs(max_z - min_z) / 2);
			for (GLfloat& i : scale)
				i = k * axis_shift;
		}
		...
		glBufferData(GL_ARRAY_BUFFER, vertex.size() * sizeof(vertex[0]), vertex.data(),
					 GL_STATIC_DRAW);
		glBindBuffer(GL_ARRAY_BUFFER, 0);
		glGenBuffers(1, &colorVBO);
		glBindBuffer(GL_ARRAY_BUFFER, colorVBO);
		glBufferData(GL_ARRAY_BUFFER, color.size() * sizeof(color[0]), color.data(),
					 GL_STATIC_DRAW);
		glBindBuffer(GL_ARRAY_BUFFER, 0);
		glGenBuffers(1, &indexEBO);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indexEBO);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, index.size() * sizeof(index[0]), index.data(),
					 GL_STATIC_DRAW);
...
vector<point_3d> get_Aizawa_attractor(int iterations) {
	...
	for (int i = 0; i < iterations; i++) {
		float dx = (z - b) * x - d * y;
		float dy = d * x + (z - b) * y;
		float dz = c + a * z - (pow(z, 3) / 3) - (pow(x, 2) + pow(y, 2)) * (1 + e * z) +
				   f * z * (pow(x, 3));
		x += dx * dt;
		y += dy * dt;
		z += dz * dt;
...
// '+', '-' scale
// '1', '2', '3' projections
// KEY_UP, KEY_DOWN, KEY_LEFT, KEY_RIGHT - rotation
int main(int argc, char* argv[]) {
	...
	V.init(get_Lorenz_attractor(5000), 1);
	// or V.init(get_Lorenz_attractor_mod_2(10000));
	// or V.init(get_Aizawa_attractor(10000), 2, 0.25, true);
